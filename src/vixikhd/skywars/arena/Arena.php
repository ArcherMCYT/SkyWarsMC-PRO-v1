...
class Arena implements Listener {
    ...
    public function joinToArena(Player $player, bool $force = false) {
        ...
        $inv->setItem(0, Item::get(Item::PAPER)->setCustomName("§r§eChange map\n§7[Use]"));
        if($this->plugin->kitManager instanceof KitManager) {
            $inv->setItem(4, Item::get(Item::FEATHER)->setCustomName("§r§eSelect kit\n§7[Use]"));
        }
 ...
 public function onVoid(PlayerMoveEvent $event) {
    ...
    public function onProjectile(EntityDamageEvent $event) {
       if($event instanceof EntityDamageByChildEntityEvent) {
            $entity = $event->getEntity();
            $this->broadcastMessage(Lang::getMsg("arena.projectile", [$entity->getName(), $entity->getHealth()]));
       }
     }
