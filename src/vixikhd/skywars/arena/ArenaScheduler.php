...
class ArenaScheduler extends Task {
    ...
    public function sendScoreboard(Player $player) {
        ...
        $fixFormatting = function (Player $player, string $text): string {
            try {
                return str_replace(
                    ["{%line}", "{%players}", "{%maxPlayers}", "{%spectators}", "{%kit}", "{%neededPlayers}", "{%time}", "{%chest}", "{%map}", "{%kills}"],
                    ["\n", (string)count($this->plugin->players), (string)$this->plugin->data["slots"], (string)count($this->plugin->spectators), (isset($this->plugin->kits[$player->getName()]) ? $this->plugin->kits[$player->getName()]->getName() : "---"), (string)$this->plugin->data["pts"], $this->getCalculatedTimeByPhase(), $this->getCalculatedTimeByRefill(), $this->plugin->level->getFolderName(), $this->plugin->kills[$player->getName()]], $text);
            }
